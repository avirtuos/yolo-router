...
/* Available-at-reject histogram: shows how many other targets had capacity when a request was rejected.
   Render as a discrete bar chart so each integer count is shown as a labeled x tick. */
(function() {{
  const arr = Array.isArray(availCounts) ? availCounts : [];
  const minVal = arr.length ? Math.min(...arr) : 0;
  const maxVal = arr.length ? Math.max(...arr) : 0;
  // Include zero bucket explicitly
  const start = Math.min(0, minVal);
  const bins = [];
  for (let v = start; v <= maxVal; v++) bins.push(v);
  const counts = bins.map(b => arr.filter(x => x === b).length);
  ...
  Plotly.newPlot('perTargetTotals', [{{
    ...
    title: 'Available Targets at Reject (counts per available-targets)',
    xaxis: {{ 
      title: '# other targets with available capacity', 
      tickmode: 'array', 
      tickvals: bins, 
...
/* Combined rates chart: retries/sec & rejects/sec plotted against a secondary (right) y-axis so they can use a different scale than requests/sec */
